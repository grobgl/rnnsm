...
_RESULT_PATH = '../../results/churn/aalen_additive_log'
...
def showJointPlot():
    ...
    df = pd.DataFrame()
    df['predicted'] = np.exp(pred_val[observed]) - 1
    df['actual'] = data.split_val['y'][observed]
    # df['actual'] = np.log(data.split_val['y'][observed] + 1)

    sns.jointplot('actual', 'predicted', data=df, kind='resid', size=figsize(.5,.5)[0])
    plt.show()
