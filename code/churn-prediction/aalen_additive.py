...
_RESULT_PATH = '../../results/churn/aalen_additive/'
...
# predPeriodHours = 2094

class AalenChurnModel:
    def __init__(self, data):
        self.cf = AalenAdditiveFitter(coef_penalizer=1)
        self.data = data
    ...
    def predict_proba(self, df):
        # pred = self.cf.predict_expectation(self.data.split_val_df)
        # predRt = (pred.values.reshape(-1) - self.data.split_val_unscaled_df.recency.values.reshape(-1))
        # minRt = predRt.min()
        # predRt += minRt
        # return churned.reshape(-1)
