...
#             self.y_test = self.y_test.reshape(self.y_test.shape + (1,))
    ...
    def weighted_mean_squared_error(self, y_true, y_pred):
        ...
        return (K.sum(K.square(y_pred - y_true), 2) * n) #/ K.sum(n)
        # return K.sum(K.prod(K.square(y_pred - y_true), n))# / K.sum(n)

    # def get_rmse_days(self, dataset='val', last_only=True):
    #     if dataset=='val':
    #         x = self.x_train_val[~self.y_train_val_churned]
    #         y = self.y_train_val[~self.y_train_val_churned]
    #     else:
    #         x = self.x_test[~self.y_test_churned]
    #         y = self.y_test[~self.y_test_churned]

    #     pred = self.model.predict(x)

    #     if last_only and self.predict_sequence:
    #         pred = pred[:,-1]
    #         y = y[:,-1]
    #     elif self.predict_sequence:
    #         mask = y != 0
    #         pred = pred[mask]
    #         y = y[mask]

    #     rmse_days = np.sqrt(mean_squared_error(pred.ravel(), y.ravel()))
    #     return rmse_days
    ...
    def get_scores(self, dataset='val'):
        ...
        rmse_days = np.sqrt(mean_squared_error(pred[~churned], y[~churned]))

        rtd_ind = self.features.index('startUserTimeDays')
        ret_time_days_pred = self.x_train_val_unscaled[:,-1,rtd_ind] + pred
        ret_time_days_true = self.x_train_val_unscaled[:,-1,rtd_ind] + y
    ...
    def fit_model(self, initial_epoch=0):
        ...
        self.model.fit(self.x_train_train_ret, self.y_train_train_ret, batch_size=100, epochs=500, validation_split=0.2, verbose=0, initial_epoch=initial_epoch
              , callbacks=[
